    ...
    public function index()
    {
        ...
        if($dbCheck1 === true){
            $checkIn1 = count(DB::select(
                        "SELECT afh.check_in
                        FROM absensi_fingerprint.pegawais p, absensijdo_frhistory.$dbName1 afh
        ...
        if($dbCheck2 === true){
            $checkIn2 = count(DB::select(
                        "SELECT afh.check_in
                        FROM absensi_fingerprint.pegawais p, absensijdo_frhistory.$dbName2 afh
                        WHERE p.pid = afh.pid AND MONTH(afh.sync_date) = '$month2' AND afh.check_in IS NOT NULL
                        ORDER BY afh.id DESC"
                    ));
        ...
        if($dbCheck3 === true){
            $checkIn3 = count(DB::select(
                        "SELECT afh.check_in
                        FROM absensi_fingerprint.pegawais p, absensijdo_frhistory.$dbName3 afh
        ...
        if($dbCheck4 === true){
            $checkIn4 = count(DB::select(
                        "SELECT afh.check_in
                        FROM absensi_fingerprint.pegawais p, absensijdo_frhistory.$dbName4 afh
